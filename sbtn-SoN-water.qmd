...
pacman::p_load("here", "dplyr", "magrittr", "sf", "raster", "exactextractr", "skimr", "readr", "tidyr")
```
...
#### Baseline water stress [Hofste et al. 2019](https://www.wri.org/data/aqueduct-global-maps-30-data)
```{r}
bws <- st_read(here(my_lib, "aqueduct30_V01_2019", "baseline", "annual", "arcmap", "y2019m07d12_aqueduct30_v01.gdb"),
               layer = "annual")
```

#### Water depletion [Brauman et al. 2016](http://www.earthstat.org/water-depletion-watergap3-basins/)
```{r}
wdp <- raster(here(my_lib, "WaterDepletion_WaterGap3", "WaterDepletionCat_WG3.tif"))
```

#### Blue water scarcity [Mekonnen & Hoekstra 2016](https://www.science.org/doi/10.1126/sciadv.1500323)
```{r}
...
#### Coastal Eutrophication Potential [Hofste et al. 2019](https://www.wri.org/data/aqueduct-global-maps-30-data)
```{r}
cep <- bws
```

#### Water Quality Index [Damania et al. 2019](https://openknowledge.worldbank.org/handle/10986/32245)
```{r}
...
## Clean and Normalize data to 1-5 values
...
### Identify which layer is the max/min value
...
```{r}
son_wa %>%
  #dplyr::select(bws_n, wdp_n, wsb_n, wa_min:wa_n_nas) %>%
  st_drop_geometry %>%
  skim()
```
