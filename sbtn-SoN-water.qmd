...
pacman::p_load(here, dplyr, magrittr, sf, terra, stars, exactextractr, skimr, readr, tidyr, openxlsx)
```
...
```{r}
wsb <- st_read(here("inputs", "blue_water_scarcity", "Report53-BlueWaterScarcity-ArcGIS-ShapeFile", "Monthly_WS_GRDC_405_basins.shp"))
```
...
```{r}
wsb_n <- wsb |> 
  rasterize(rast(wsb, res = 0.08333), field = "Wsavg", fun = "mean") |> # ~10km
  exact_extract(h6, fun = c("mean", "median", "quantile", "majority", "coefficient_of_variation"), quantiles = 0.75,
                       append_cols = "HYBAS_ID", progress = TRUE)
...
son_water |> 
  ...
  mapview::mapview(
    ...
    col.regions = c("#e9ffbe", "#ffff73", "#ffd700", "#ff5500", "#cc0000"),
    lwd = 0.2,
    alpha.regions = 0.9
  )
```
