...
bws <- st_read(here(my_lib, "aqueduct30_V01_2019", "baseline", "annual", "arcmap", "y2019m07d12_aqueduct30_v01.gdb"),
               layer = "annual") %>%
  dplyr::select(string_id, aq30_id, pfaf_id, bws_raw:bws_label, geometry = Shape)
```
...
cep <- st_read(here(my_lib, "aqueduct30_V01_2019", "baseline", "annual", "arcmap", "y2019m07d12_aqueduct30_v01.gdb"),
               layer = "annual") %>%
  dplyr::select(string_id, aq30_id, pfaf_id, cep_raw:cep_label, geometry = Shape)
```
...
wdp_stack <- stack(wdp, wdp_n)
names(wdp_stack) <- c("wdp_cat", "wdp_n")

rm(wdp_rcl)
```

```{r}
wdp_n <- exact_extract(wdp_stack, h6, fun = c("majority", "coefficient_of_variation"), # use mean or 75th instead?
...
wdp_n <- wdp_n %>%
  mutate(
    wdp_cat = majority.wdp_cat,
    wdp_n = majority.wdp_n,
    wdp_label = case_when(
      wdp_cat == 1 ~ "<=5% annual depletion",
      wdp_cat == 2 ~ ">5–25% annual depletion",
      wdp_cat == 5 ~ ">25–75% annual depletion, Dry-Year",
      wdp_cat == 6 ~ ">25–75% annual depletion, Seasonal",
      wdp_cat == 7 ~ ">75–100% annual depletion",
      wdp_cat == 8 ~ ">100% annual depletion"
    ),
    wdp_cov = coefficient_of_variation.wdp_n
  ) %>%
  dplyr::select(HYBAS_ID, wdp_cat:wdp_cov)
```

#### Blue water scarcity
```{r}
wsb_n <- exact_extract(wsb, h6, fun = c("mean", "stdev", "variance", "median", "coefficient_of_variation"),
                       append_cols = "HYBAS_ID", progress = TRUE)
...
wsb_n <- wsb_n %>%
  mutate(
    ...
    wsb_cov = coefficient_of_variation
  ) %>%
  dplyr::select(HYBAS_ID, wsb_n:wsb_cov)
```
