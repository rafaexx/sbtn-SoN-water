...
pacman::p_load(here, dplyr, readr, sf, stars, terra, fasterize, purrr, exactextractr, skimr, mapview, openxlsx)
```
...
fasterize_son_water <- function(var){
  fasterize(son_water, raster(son_water, res = 0.01), field = var, fun = "max") |> 
    writeRaster(here("outputs", "raster", paste0(var, ".tif")), overwrite = TRUE)
}
```
...
```{r}
stack_wa <- c(
  rast(here("outputs", "raster", "bws_n.tif")),
  rast(here("outputs", "raster", "wdp_n.tif")),
  rast(here("outputs", "raster", "wsb_n.tif"))
)
...
```{r}
stack_wp <- c(
  rast(here("outputs", "raster", "cep_n.tif")),
  rast(here("outputs", "raster", "nox_n.tif")),
  rast(here("outputs", "raster", "pgp_n.tif"))
)
...
stack_wa_adm0 <- stack_wa |> 
  exact_extract(adm0, fun = "weighted_mean", weights = resample(weights_wa, stack_wa), default_weight = 0, append_cols = "GID_0", progress = TRUE) 
```

```{r}
stack_wp_adm0 <- stack_wp |> 
  exact_extract(adm0, fun = "weighted_mean", weights = resample(weights_wp, stack_wp), default_weight = 0, append_cols = "GID_0", progress = TRUE) 
```

*Sub-national level*
```{r}
stack_wa_adm1 <- stack_wa |> 
  exact_extract(adm1, fun = "weighted_mean", weights = resample(weights_wa, stack_wa), default_weight = 0, append_cols = "GID_1", progress = TRUE) 
```

```{r}
stack_wp_adm1 <- stack_wp |> 
  exact_extract(adm1, fun = "weighted_mean", weights = resample(weights_wp, stack_wp), default_weight = 0, append_cols = "GID_1", progress = TRUE) 
```
...
```{r}
son_water_adm0 <- adm0 |> 
  left_join(stack_wa_adm0, by = "GID_0") |>
  left_join(stack_wp_adm0, by = "GID_0") |>
  get_max()
```
...
```{r}
son_water_adm1 <- adm1 |> 
  left_join(stack_wa_adm1, by = "GID_1") |>
  left_join(stack_wp_adm1, by = "GID_1") |>
  get_max()
```
...
), file = here("outputs", "adm_level", "sbtn_son_water_adm_level.xlsx"), rowNames = FALSE)
```

*National level*
```{r}
write_rds(son_water_adm0, here("outputs", "adm_level", "sbnt_son_water_adm0.rds"))
```

```{r}
# st_write(son_water_adm0, here("outputs", "adm_level", "sbnt_son_water_adm0.shp"), layer_options = "ENCODING=UTF-8", delete_layer = TRUE)
```

*Sub-national level*
```{r}
write_rds(son_water_adm1, here("outputs", "adm_level", "sbnt_son_water_adm1.rds"))
```

```{r}
# st_write(son_water_adm1, here("outputs", "adm_level", "sbnt_son_water_adm1.shp"), layer_options = "ENCODING=UTF-8", delete_layer = TRUE)
```

## 7. View outputs

```{r}
pacman::p_load(here, dplyr, readr, sf, mapview)
```

```{r}
son_water_adm0 <- read_rds(here("outputs", "adm_level", "sbnt_son_water_adm0.rds"))
son_water_adm1 <- read_rds(here("outputs", "adm_level", "sbnt_son_water_adm1.rds"))
```
...
*National level*
```{r}
son_water_adm0_simplified <- son_water_adm0 |> st_transform("EPSG:8857") |> st_simplify(dTolerance = 1000) |> st_transform("EPSG:4326")

mapview_custom(son_water_adm0_simplified, "Water Availability (Max)", "wa_max") +
mapview_custom(son_water_adm0_simplified, "Water Pollution (Max)", "wp_max")
```

*Sub-national level*
```{r}
son_water_adm1_simplified <- son_water_adm1 |> st_transform("EPSG:8857") |> st_simplify(dTolerance = 1000) |> st_transform("EPSG:4326")

mapview_custom(son_water_adm1_simplified, "Water Availability (Max)", "wa_max") +
mapview_custom(son_water_adm1_simplified, "Water Pollution (Max)", "wp_max")
```
