...
### 3.1. WaterGap3 water consumption as weights for water depletion (provided by the authors)
...
### 3.2. Water Footprint Network Blue water footprint as weights for blue water scarcity (provided by the authors)
...
### 3.2. N and P fertilizer use as weights for water pollution indicators [Lu & Tian 2016](https://doi.pangaea.de/10.1594/PANGAEA.863323)

```{r}
n_use <- rast(here("inputs", "n_p_fertilizer_use", "Nfer_ASCII", "nfery2013.asc"))
```

```{r}
p_use <- rast(here("inputs", "n_p_fertilizer_use", "Pfer_ASCII", "pfery2013.asc"))
```
...
```{r}
wp_weights <- app(c(min_max_scale(n_use), min_max_scale(p_use)), fun = mean, na.rm = TRUE)
```
...
key_descriptions <- tribble(
  ...
  "_agr","Aggregation at adm level weighted by N & P fertilizer use in agricultural practices (https://doi.org/10.5194/essd-9-181-2017)",
  "","",
