...
```{r}
bws <- st_read(here("inputs", "aqueduct40", "GDB", "Aq40_Y2023D07M05.gdb"), layer = "baseline_annual")
```
...
```{r}
wsb <- rast(here("inputs", "blue_water_scarcity", "raster", "ws_avg.tif"))
```
...
```{r}
write_rds(wa_n, here("outputs", "wa_n_v1_1.rds"))
```
...
```{r}
write_rds(wp_n, here("outputs", "wp_n_v1_1.rds"))
```
...
```{r}
wa_n <- read_rds(here("outputs", "wa_n_v1_1.rds"))
```
...
```{r}
wp_n <- read_rds(here("outputs", "wp_n_v1_1.rds"))
```
...
```{r}
write_rds(son_water, here("outputs", "sbtn_son_water_v1_1.rds"))
```

```{r}
# st_write(son_water, here("outputs", "sbtn_son_water_v1_1.shp"), layer_options = "ENCODING=UTF-8", delete_layer = TRUE)
```

```{r}
# write.xlsx(list(
#   "SBTN SoN Water" = st_drop_geometry(son_water)
# ), file = here("outputs", "sbtn_son_water_v1_1.xlsx"), rowNames = FALSE)
```
